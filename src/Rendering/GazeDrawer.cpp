...
namespace eyegui
{
    ...
    void GazeDrawer::update(int gazeX, int gazeY, float tpf)
    {
        ...
        float minGazePointRelativeSize = mpGUI->getConfig()->getValue(StyleValue_float::GazeVisualizationMinSize)->get();
        float maxGazePointRelativeSize = mpGUI->getConfig()->getValue(StyleValue_float::GazeVisualizationMaxSize)->get();
        ...
        else if(glm::distance(gazePoint.point, mPoints.back().originalPoint) > calculateDiameterOfGazePoint(mPoints.back()) / 2.0f)
        {
            ...
            if(mPoints.back().focus.getValue() < mpGUI->getConfig()->getValue(StyleValue_float::GazeVisualizationRejectThreshold)->get())
            {
        ...
        else
        {
            ...
            mPoints.back().focus.update(tpf / mpGUI->getConfig()->getValue(StyleValue_float::GazeVisualizationFocusDuration)->get());
        ...
        {
            mPoints[i].alpha.update(-tpf / mpGUI->getConfig()->getValue(StyleValue_float::GazeVisualizationFadeDuration)->get());
    ...
    void GazeDrawer::draw() const
    {
        ...
        for(uint i = 0; i < mPoints.size() - 1; i++)
        {
            ...
            mpLine->getShader()->fillValue("color", mpGUI->getConfig()->getValue(StyleValue_vec4::GazeVisualizationColor)->get());
        ...
        for (const GazePoint& rGazePoint : mPoints)
        {
            ...
            mpCircle->getShader()->fillValue("color", mpGUI->getConfig()->getValue(StyleValue_vec4::GazeVisualizationColor)->get());

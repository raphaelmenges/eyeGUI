...
namespace eyegui
{
    ...
    {
        return mRelativeScale + (mAdaptiveScale.getValue() * mpLayout->getConfig()->getValue(StyleType_float::MaximalAdaptiveScaleIncrease)->get());
    }
    ...
    float Element::update(float tpf, float alpha, Input* pInput, float dim)
    {
        ...
        // Activity animation
        mActivity.update(tpf / mpLayout->getConfig()->getValue(StyleType_float::AnimationDuration)->get(), !mActive);
        ...
        {
            // Undim it
            mDim.update(-tpf / mpLayout->getConfig()->getValue(StyleType_float::DimDecreaseDuration)->get());
        }
        else if (mDimming)
        {
            if (penetrated)
            ...
            {
                // Dim it
                mDim.update(tpf / mpLayout->getConfig()->getValue(StyleType_float::DimIncreaseDuration)->get());
            }
        }
        else
        {
		...
		{
			mFlash.update(-tpf / mpLayout->getConfig()->getValue(StyleType_float::FlashDuration)->get());
		}
        ...
        if (mAdaptiveScaling)
        {
            ...
            {
                // Scale it up
                mAdaptiveScale.update(tpf / mpLayout->getConfig()->getValue(StyleType_float::AdaptiveScaleIncreaseDuration)->get());
            }
            else
            {
                // Scale it down
                mAdaptiveScale.update(-tpf / mpLayout->getConfig()->getValue(StyleType_float::AdaptiveScaleDecreaseDuration)->get());
            }
        }
        else
        {
        ...
        // Marking
        mMark.update(tpf / mpLayout->getConfig()->getValue(StyleType_float::AnimationDuration)->get(), !mMarking);
        ...
        if (mupReplacedElement.get() != NULL)
        {
            float replacedAlpha = mAlpha * (mupReplacedElement->getAlpha()
                - (tpf / mpLayout->getConfig()->getValue(StyleType_float::AnimationDuration)->get()));

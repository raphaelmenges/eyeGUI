...
namespace eyegui
{
    ...
    void WordSuggest::specialDraw() const
    {
        ...
        if (!mSuggestions.empty())
        {
            ...
            for (const std::unique_ptr<TextSimple>& rSuggestion : mSuggestions)
            {
                rSuggestion->draw(getStyleValue(property::Color::FontColor), mAlpha);
            }
            ...
            {
                rChosenSuggestion.upText->draw(getStyleValue(property::Color::FontColor), rChosenSuggestion.alpha);
            }
